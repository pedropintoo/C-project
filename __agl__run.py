...
import numpy as np
from antlr4 import *
from antlr4 import ANTLRInputStream
...
if __name__ == "__main__":
    ...
    # Play with XAGL file
    #file = var__agl__s2
    vars1 = {"root":root, "m":v98,"v":v101}
    vars0 = transform_dict(vars1)

    #visitor0 = Semantic(vars0)
    visitor1 = Interpreter(vars1)
    #input_stream = FileStream(file) if file.endswith('.xagl') else ANTLRInputStream(file)
    input_stream = InputStream(var__agl__s2)
    print(input_stream)
    lexer = XAGLLexer(input_stream)
    stream = CommonTokenStream(lexer)
    parser = XAGLParser(stream)
